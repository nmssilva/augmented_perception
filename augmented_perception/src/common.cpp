...
void AssociateObjects(vector<t_listPtr> &list, vector<t_objectPtr> &objects, t_config &config, t_flag &flags)
{
  ...
  for (uint i = 0; i < list.size(); i++)
  {
    ...
    if (association_found)
    {
      ...
      for (uint e = 0; e < list.size(); e++)
      {
        ...
        dist = point2point_distance(objects[min_index]->cx, objects[min_index]->cy, list[e]->position.predicted_x,
  ...
  for (uint g = 0; g < objects.size(); g++)
  {
    ...
    {
      dist_to_object =
          point2point_distance(objects[g]->cx, objects[g]->cy, list[i]->measurements.x, list[i]->measurements.y);
...
void MotionModelsIteration(vector<t_listPtr> &list, t_config &config)
{
  ...
  for (uint i = 0; i < list.size(); i++)
  {
    ...
    if ((int)list[i]->id == select_object)
    {
      ...
      fprintf(fp, "%2.6f %2.6f %2.6f %2.6f %2.6f %2.6f %2.6f %2.6f ", cvgpx, cvgpy, cvgvx, cvgvy, cagpx, cagpy, cagvx,
              cagvy);
    }
    ...
    {
      double xQcv[] = { pow(xAcv, 2) * pow(dt, 3) / 3, pow(xAcv, 2) * pow(dt, 2) / 2, pow(xAcv, 2) * pow(dt, 2) / 2,
                        pow(xAcv, 2) * pow(dt, 1) };
      double yQcv[] = { pow(yAcv, 2) * pow(dt, 3) / 3, pow(yAcv, 2) * pow(dt, 2) / 2, pow(yAcv, 2) * pow(dt, 2) / 2,
                        pow(yAcv, 2) * pow(dt, 1) };

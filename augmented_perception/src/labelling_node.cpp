...
void filter_pc() {
	...
	if (click_on) {
		...
		for (int i = 0; i < pointDatapcl.points.size(); i++) {
			if (pointDatapcl.points[i].y > pointDatapcl.points[i].x * (proportion + 0.05) ||
				pointDatapcl.points[i].y < pointDatapcl.points[i].x * (proportion - 0.05) ||
...
void image_cb_TemplateMatching(const sensor_msgs::ImageConstPtr &msg) {
	try {
		...
		if (c == 'p') {
			...
			for (std::map < unsigned int, std::vector < BBox > > ::iterator it = file_map.begin(); it !=
																								   file_map.end();
			++it) {
				// Skipped frames part
...
int main(int argc, char **argv) {
	...
	// ros::Subscriber sub_scan_0_mtt = nh.subscribe("/filter/scan0", 1,
	// scan_0_cb);
